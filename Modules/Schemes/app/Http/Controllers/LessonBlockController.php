...
class LessonBlockController extends Controller
{
  ...
  public function index(Course $course, Unit $unit, Lesson $lesson)
  {
    ...
    if (
      (int) $lessonModel->unit?->course_id !== (int) $course->id ||
      (int) $lessonModel->unit_id !== (int) $unit->id
    ) {
      return $this->error(__("messages.lesson_blocks.lesson_not_in_course"), 404);
    }
    ...
    if (!$course) {
      return $this->error(__("messages.lesson_blocks.course_not_found"), 404);
    }
    ...
    if (!$authorized) {
      return $this->error(__("messages.lesson_blocks.no_view_access"), 403);
    }
  ...
  public function store(LessonBlockRequest $request, Course $course, Unit $unit, Lesson $lesson)
  {
    ...
    if (!$authorized) {
      return $this->error(__("messages.lesson_blocks.no_manage_access"), 403);
    }
    ...
    return $this->created(["block" => $block], __("messages.lesson_blocks.created"));
  }
  ...
  public function update(
    ...
    if (!Gate::forUser($user)->allows("update", $lessonModel)) {
      return $this->error(__("messages.lesson_blocks.no_update_access"), 403);
    }
    ...
    return $this->success(["block" => $updated], __("messages.lesson_blocks.updated"));
  }
  ...
  public function destroy(Course $course, Unit $unit, Lesson $lesson, LessonBlock $block)
  {
    ...
    if (!Gate::forUser($user)->allows("delete", $lessonModel)) {
      return $this->error(__("messages.lesson_blocks.no_delete_access"), 403);
    }
